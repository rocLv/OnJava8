...
C风格的编程语言，特别是C++，一般都将程序性能置于程序安全性之上。开发Java程序之所以比开发C++程序快很多(在大多数情况下，Java开发效率大约是C++的两倍)，是因为Java的安全性，包括垃圾回收和更加先进的类型检查。通过将单元测试集成到构建过程中，你扩大了这个安全保障，因而有了更快的开发效率。当发现设计或实现的缺陷时，可以更容易、更大胆地重构你的代码。
